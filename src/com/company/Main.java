...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (!line.equals("q")) {
                try {
                    result = compute(line);
                } catch (ChybiDruhaZavorka chybiDruhaZavorka) {
                    System.out.println("Chybí druhá závorka");
                } finally {
    ...
    private static int compute(String formula) throws ChybiDruhaZavorka {
    ...
    private static String computeBraces(String formula) throws ChybiDruhaZavorka {
        if (!formula.contains("(")) return formula;
        else {
            int start = formula.indexOf("(");
            int end = formula.indexOf(")");
            if (end == -1) throw new ChybiDruhaZavorka();
